  ...
  fetch(
    ...
    .then((newsData) => {
      ...
      document.querySelector(
        ".pagination_section"
      ).innerHTML = `<a href="#" onclick="pageChange(-1)"><< Previous</a>`;
      for (let i = 0; i <= newsData.length / 10; i++) {
        if (i == pageNumber) {
          document.querySelector(
            ".pagination_section"
          ).innerHTML += `<a href="#" onclick="newsPage(${i})" class="active" >${
            i + 1
          }</a>`;
          continue;
        }
        document.querySelector(
          ".pagination_section"
        ).innerHTML += `<a href="#" onclick="newsPage(${i})">${i + 1}</a>`;
      }
      document.querySelector(
        ".pagination_section"
      ).innerHTML += `<a href="#" onclick="pageChange(1)">Next >></a>`;
